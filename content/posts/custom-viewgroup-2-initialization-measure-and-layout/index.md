...
## 总结
我们从子控件初始化开始，讲到如何利用 `measureChild` 实现测量，以及如何布局。几乎把（我遇到的）常见的业务 UI 都包含在内。你能看到这里，我相信即便没有完全掌握渲染流程，也了解了六七分了😋 。但是仅有理论总归是不够的，我后续将会分享实践相关的文章，用一些简单方便入手的例子，帮助你快速掌握自定义 ViewGroup 的渲染流程。期待与你再见～