...
- **Cache key in dir + URL**: original plan stored `PROCESSOR_VERSION` in `meta.json` only; cache + public dirs were keyed by `hash`. A version bump would leave stale derivatives next to new ones in the same dir, and unchanged URLs would let CDN/browser caches serve old bytes. Cache + public are now keyed by `cacheKey` (= `<hash>-v<version>`); bumping version starts a fresh dir + URL. Old dirs become orphans, removed by `gcCacheSync` after 60 days.
- **Test fixture location**: `tea-heic-with-live-photo.heic` originally lived in the user's draft post (`content/posts/2026-04-25-year-end-summary/`). When that post turned out to need to stay local-only until the user finishes writing, the HEIC fixture was moved to `tests/fixtures/sample.heic` and a tiny `tests/fixtures/sample.png` was generated for non-HEIC pass-through coverage. E2E tests (`media-output`, `media-cache-idempotence`) now create an ephemeral post under `content/posts/_e2e_*/` via `tests/_helpers/temp-post.mjs` for the duration of the test run. The `dist/year-end-summary/` path no longer appears in tests.
